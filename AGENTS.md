...
- REQUIRED: finish and validate the public read-only application; complete user
  and code documentation; add a local Gold audit view; perform the priority
  visual review; publish a reproducible read-only web version; and finish final
  validation, screenshots and the written TFM.
- CONDITIONAL: add fields only after an evidence audit shows that they are
  reliable, traceable and bounded. Streamlit must never read Silver directly.
- POST-TFM: persistent reports, an administrative application, authentication
  and authorisation, assisted corrections and operational automation.

- Classify every new proposal as BLOCKER, REQUIRED, CONDITIONAL, OPTIONAL, or
  POST-TFM. Do not start CONDITIONAL, OPTIONAL, or POST-TFM work while BLOCKER
...
- Do not treat Gold as formally validated until it has been rebuilt and checked
  against Silver generated by the frozen extraction configuration.
...
- Streamlit is a read-only presentation layer over validated, versioned Gold.
  It must not read Silver, execute the pipeline, call external services or
  provide write or correction paths.
