...
- [Resultados finales V2](evaluation/FINAL_HOLDOUT_V2_RESULTS.md): protocolo,
  resultados, procedencia y límites de la evaluación.
...
`data/bronze/localizaciones_ine/` contiene exclusivamente dos CSV públicos de
referencia territorial necesarios para los notebooks 06 y 10. No se distribuyen
otros datos Bronze ni materializaciones Silver; la aplicación consume únicamente
el paquete Gold contractual versionado.
...
- [código del evaluador V2-B](../evaluation/final_holdout_v2/evaluator.py);
- [controlador de ejecución primaria](../evaluation/primary_execution/).
...
Los identificadores Git `282de815…`, `2c0632b…`, `7108d5d4…`, `5871411…` y
`a8af57e…` son referencias históricas internas del repositorio privado de
...
Los documentos `FINAL_*.md` incluidos directamente en `docs/` aportan evidencia
no redundante sobre la construcción del corpus, W14, las correcciones humanas y
la materialización final. Los planes, preflights, registros de incidencias y
checklists internos permanecen fuera de la distribución pública.

Las menciones a rutas `runs/...` en esos informes son identificadores históricos
de procedencia de artefactos operativos no distribuidos: no son enlaces, no se
requieren para ejecutar la aplicación y no justifican publicar `runs/`.
...
El PDF público se genera desde las fuentes LaTeX sin la firma privada; el PDF
histórico de la raíz no forma parte de esta distribución.
