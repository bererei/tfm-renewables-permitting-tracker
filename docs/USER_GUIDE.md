...
2026-09-13, realizada en la rama privada histórica `tfm-evaluation`. No forman
parte del sistema que produjo los resultados: el ref privado histórico
`tfm-final` permanece en
`282de815bea4e248bdcba2c655e3ee078cb58a49`. Ni la rama ni ese commit pertenecen
a la nueva historia pública o son resolubles desde ella. Este cambio no altera las versiones
...
La aplicación de consulta nunca edita Gold. Un backend administrativo persistente
permanece POST-TFM.
...
Los scopes públicos del corpus final P2 y su identidad están declarados en el
[manifest versionado](../config/evaluation/final_p2_execution_scopes_v1/manifest.json);
los comandos controlados se describen en esta misma sección y los contratos se
aplican desde la CLI versionada.
...
`evaluation/final_holdout_v2/`. El sistema evaluado sigue siendo exactamente el
identificado por el ref privado histórico `tfm-final` en
`282de815bea4e248bdcba2c655e3ee078cb58a49`; no es un ref del repositorio
público.
...
forma parte del código distribuido y el guard de uso está implementado. La
trazabilidad de la ejecución y del evaluador se conserva en la documentación
pública de evaluación. El worktree productivo detached, la carga explícita de
la API key y la revisión del preflight siguen siendo controles obligatorios.
La ejecución real sigue sin autorizarse por el mero hecho de disponer de estas
herramientas.
...
No lo regeneres ni sobrescribas. Sigue validando contra el código histórico
registrado en su fixture; el checkout actual lo rechaza por identidad distinta,
como debe. Un nuevo freeze debe publicarse siempre en otro destino y conservar
sus propios valores verificados:
...
El controlador público en [`evaluation/primary_execution/`](../evaluation/primary_execution/)
implementa preparación, reanudación y finalización por documento. La preparación
verifica los 48 BOE y sus hashes, y crea metadata, scopes y journal sin llamar
al modelo. La ejecución definitiva, sus identidades y su resultado terminal se
documentan en los
[resultados finales V2](evaluation/FINAL_HOLDOUT_V2_RESULTS.md). Los nombres
`runs/...` conservados en esta guía son provenance de artefactos operativos no
distribuidos;
...
REPO=/path/to/tfm-renewables-permitting-tracker
cd "$REPO"
env -u PYTHONPATH PYTHONDONTWRITEBYTECODE=1 \
...
[contrato V2](evaluation/FINAL_HOLDOUT_EVALUATION_CONTRACT_V2.md), el
[código público del evaluador](../evaluation/final_holdout_v2/evaluator.py) y los
[resultados finales](evaluation/FINAL_HOLDOUT_V2_RESULTS.md).
...
el alcance de esta distribución. El sistema evaluado, los resultados y sus
límites se documentan en los
[resultados finales V2](evaluation/FINAL_HOLDOUT_V2_RESULTS.md); estas mejoras
permanecen fuera del producto público entregado.
...
- [Contrato Silver](architecture/silver_extraction_contract.md): esquema,
  validación y salvaguardas de materialización.
- [Resultados finales V2](evaluation/FINAL_HOLDOUT_V2_RESULTS.md): sistema,
  evaluador, métricas, procedencia y límites.
- [Especificación del producto](APP_PRODUCT_SPEC.md): alcance, métricas y mapa.
