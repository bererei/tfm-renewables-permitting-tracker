...
| --- | --- |
| Frozen production commit, private historical branch/ref `tfm-final` | `282de815bea4e248bdcba2c655e3ee078cb58a49` |
...
El evaluador definitivo pertenece al checkout privado histórico
`2c0632b89d59ef3f7e25c24ee61e74ca3ac3c6f1`, no resoluble desde el repositorio
público. Su freeze precede a la
...
Los siguientes comandos se ejecutaron históricamente desde el entorno privado
preparado, sin red ni cambios de dependencias, antes de editar documentación.
Usaron el checkout privado del evaluador `2c0632b…` y su entorno asociado. Las
rutas `runs/...` no se distribuyen, por lo que el bloque conserva provenance de
la validación original y no es una receta ejecutable desde un clon público. No
requirieron API key ni cargar `.env`.
...
Los documentos históricos de implementación describen aquellos bloques; este
informe, el contrato V2 y la ejecución primaria documentada constituyen la
declaración pública del estado final. El contrato metodológico permanece intacto.

## Memory integration map

El documento activo [tfm_report_bgd.tex](../tfm_report/tfm_report_bgd.tex)
integra los capítulos `01`–`09` y `chapters/anexos.tex`. Los borradores
históricos quedan fuera de la distribución pública y no participan en la
compilación. El contenido de evaluación ya se ha incorporado
...
La compilación reproducible pública tiene 109 páginas A4. El mapa siguiente
registra la ubicación editorial final, no una nueva fase experimental ni una
...
[código del evaluador V2-B](../../evaluation/final_holdout_v2/evaluator.py) y
[guía de usuario](../USER_GUIDE.md).
Para las identidades finales de Gold, usar el manifest contractual versionado
bajo `data/gold/`, no el snapshot histórico v1 del informe downstream.
